...
def parseHoliday(filePath):
    ...
    holSet={}
    for line in f:
        if '#' in line[0:5]: continue # ignore comments if # is observed in first 5 char
        rec=line.strip().split(",")
        if len(rec) <= 1: continue #skip if empty lines
        ZENOSX.log_debug("parsing holiday: %s %s" % (rec,len(rec)))
...
"""
Usage:
go to ZENOSX.widget parent folder, and issue command:
US_ZENOSX_DIR="/path/to/ZENOSX.widget" python ZENOSX.widget/CAL.widget/cal.py 2 ZENOSX.widget/CAL.widget/HOLIDAYS/offdays.csv
this will run this script, for 2 months ahead, using the "offdays.csv" as the source of holidays
"""
def main():

    ZENOSX.log_debug("*** Starting Calendar Job ***")
